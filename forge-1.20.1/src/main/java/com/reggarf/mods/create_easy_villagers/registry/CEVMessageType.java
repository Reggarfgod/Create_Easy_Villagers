package com.reggarf.mods.create_easy_villagers.registry;
...
@Mod.EventBusSubscriber(modid = Create_easy_villagers.MODID)
public class CEVMessageType {
    ...
    public static void onPlayerJoin(PlayerEvent.PlayerLoggedInEvent event) {
        if (!(event.getEntity() instanceof ServerPlayer player) || !CreateEasyVillagersConfig.areMessagesEnabled()) return;
        ...
        if (!igData.getBoolean("create_easy_villagers_hasJoinedBefore")) {
            sendStyledMessages(player); // Send welcome + links
            igData.putBoolean("create_easy_villagers_hasJoinedBefore", true);
    ...
    private static void sendStyledMessages(ServerPlayer player) {
        ...
        Component title = Component.literal("Hello, thank you for downloading ")
                .append(Component.literal("Create: Easy Villagers")
        ...
        Component issueTracker = Component.literal(" - ")
                .append(Component.literal("Issue Tracker")
                        .setStyle(Style.EMPTY
                                .withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://github.com/Reggarfgod/Create_Easy_Villagers/issues"))
